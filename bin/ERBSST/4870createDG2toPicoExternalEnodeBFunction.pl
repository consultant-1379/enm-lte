...
# Jira	      : NETSUP-3047
# Purpose     : creates DG2toPICO ExternalENodeBFunction               
# Description : creates support for the creation of DG2 cell relations 
#               to PICO nodes
# Date        : 05-June-2015
# Who         : xkamvat
####################################################################
...
local $minimumcellsize=3; # minimum LTE node cell size to be related with PICO nodes# check, changed from 6 to 3
...
#
#
#check the major and minor calculations
...
local $pico2dg2cellrelationssize=0;
local $arrsize;
...
# check PICO2DG2 network relations file exits
...
close(FH1);
$pico2dg2cellrelationssize=@PICO2LTECELLRELATIONS;
if($pico2dg2cellrelationssize<1){
...
# CXP 903 0491-114-1 : offset PICO eNBId by number of preceding nodes in the network (8000)
...
print $LTENAME;
    ...
    ############################################
    # start set DG2toPICO ExternalENodeBFunction
    ############################################
...
#print "$ltearrayposition\t","$arrsize";     
...
$prevLTENodename=$configLTEnodename;
    ...
    print "Creating $LTENAME -> ExternalENodebFunction=$configPICOnodename\n";
    
#    $temppiconodecount+=$nodecountoffset;
     
    # build mo script
    @MOCmds=qq( CREATE
      (
      parent "ManagedElement=$LTENAME,ENodeBFunction=1,EUtraNetwork=1"
       identity $configPICOnodename
       moType ExternalENodeBFunction
       exception none
       nrOfAttributes 3
       externalENodeBFunctionId String $configPICOnodename
       eNBId Int32 $temppiconodecount
       masterEnbFunctionId String "1"
       eNodeBPlmnId Struct
        nrOfElements 3
        mcc Int32 353
         mnc Int32 57
