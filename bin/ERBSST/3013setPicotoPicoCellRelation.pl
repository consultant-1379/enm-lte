...
while ($NODECOUNT<=$PICONUMOFRBS){
    ...
    ##################################  
    $counter=1;$counter2=1;$arrcounter=0;
...
"setmoattribute:mo=\"ManagedElement=$LTENAME,ENodeBFunction=1,$picocelltype=$LTENAME-$cellcount,$picoeutranfregrelation=1,$picoeutrancellrelation=$counter2\",attributes=\"neighborCellRef (moRef)=ManagedElement=$LTENAME,ENodeBFunction=1,EUtraNetwork=1,ExternalENodeBFunction=$tempstringnode,ExternalEUtranCellFDD=$tempstringnode-$cellcount\";"
    );# end @MMLCmds
    
    $NETSIMMMLSCRIPT=&makeMMLscript("append",$MMLSCRIPT,@MMLCmds);
    $counter++;$arrcounter++;$counter2++;
