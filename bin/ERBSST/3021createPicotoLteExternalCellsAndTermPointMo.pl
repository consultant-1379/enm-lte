...
# Purpose     : creates PICO ExternalENodeBFunction external LTE cells            
...
while ($NODECOUNT<=$PICONUMOFRBS){
   ...
   while ($LTENAME=~m/$configPICOnodename/){ # match PICO node 
    ...
    @MMLCmds=(".open ".$SIMNAME,
	       ...
	       "createmo:parentid=\"ManagedElement=$LTENAME,ENodeBFunction=1,EUtraNetwork=1,ExternalENodeBFunction=$configLTEnodename\",type=\"$picotype\",name=\"$configLTEnodecellname\";"
    );# end @MMLCmds

    $NETSIMMMLSCRIPT=&makeMMLscript("append",$MMLSCRIPT,@MMLCmds);

   #do this once per external node
   if(!($prevconfigLTEnodename eq $configLTEnodename))
    {
        # build mml script
        @MMLCmds=(".open ".$SIMNAME,
               ".select ".$LTENAME,
               ".start ",
               "useattributecharacteristics:switch=\"off\";",
                "createmo:parentid=\"ManagedElement=$LTENAME,ENodeBFunction=1,EUtraNetwork=1,ExternalENodeBFunction=$configLTEnodename\",type=\"$TermPoint\",name=\"1\";",
                "setmoattribute:mo=\"ManagedElement=$LTENAME,ENodeBFunction=1,EUtraNetwork=1,ExternalENodeBFunction=$configLTEnodename,$TermPoint=1\",attributes=\"ipAddress (string)=$ipaddress\";"
        );# end @MMLCmds
print $fh "$MMLCmds[4]";
print $fh "\n";
        $NETSIMMMLSCRIPT=&makeMMLscript("append",$MMLSCRIPT,@MMLCmds);
    ...
    $prevconfigLTEnodename=$configLTEnodename;
    $picoarrayposition++;
