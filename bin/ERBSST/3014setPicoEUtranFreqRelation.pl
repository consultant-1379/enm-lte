...
while ($NODECOUNT<=$PICONUMOFRBS){
    ...
    ##################################
    # start create PICO EUtranFreqReltion
    ##################################
    # build mml script 
...
"setmoattribute:mo=\"ManagedElement=$LTENAME,ENodeBFunction=1,$picocelltype=$LTENAME-$cellcount,$picoeutranfregrelation=$cellcount\",attributes=\"eutranFrequencyRef (moRef)=ManagedElement=$LTENAME,ENodeBFunction=1,EUtraNetwork=1,EUtranFrequency=1\";"  
    );# end @MMLCmds
