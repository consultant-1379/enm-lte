...
# Purpose     : creates DG2toPICO CellRelation              
...
while ($NODECOUNT<=$DG2NUMOFRBS){
    ...
    $moline="ManagedElement=$LTENAME,ENodeBFunction=1,EUtranCellFDD=$configLTEnodecellname,EUtranFreqRelation=1";
    $moline=~s/\n//;
   ...
   # build mo script
    @MOCmds=qq( CREATE
            ...
            neighborCellRef Ref ManagedElement=$LTENAME,ENodeBFunction=1,EUtraNetwork=1,ExternalENodeBFunction=$configPICOnodename,ExternalEUtranCellFDD=$configPICOnodecellname
            );
            );# end @MO
