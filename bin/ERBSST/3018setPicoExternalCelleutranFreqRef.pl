...
# Purpose     : sets PICO External Network Cell.eutranFreqRef           
...
while ($NODECOUNT<=$PICONUMOFRBS){
    ...
    while($counter<=$PICORELATIONSNUMBER){
    	  ...
    	  "setmoattribute:mo=\"ManagedElement=$LTENAME,ENodeBFunction=1,EUtraNetwork=1,ExternalENodeBFunction=$tempstringnode,$picotype=$tempstringnode-$cellcount\",attributes=\"$eutranFrequencyRef (moRef)=ManagedElement=$LTENAME,ENodeBFunction=1,EUtraNetwork=1,EUtranFrequency=1\";" 
    );# end @MMLCmds
