...
# Purpose     : set PICO masterEUtranCellFDDId
# Jira        : NETSUP-1019
# Date        : Feb 2014
# Who         : ecasjim
####################################################################
...
local $element;
local $masterEUtranCellFDDId="masterEUtranCellFDDId";
local $localCellId="localCellId";
local $currlocalCellId;
...
while ($NODECOUNT<=$PICONUMOFRBS){
   ...
   while ($LTENAME=~m/$configPICOnodename/){ # match PICO node 
    ...
    $configLTEnodename=~ s/^\s+|\s+$//g;
    $currlocalCellId=$configLTEnodecellname;
    $currlocalCellId=~ s/.*\-//g;
...
"setmoattribute:mo=\"ManagedElement=$LTENAME,ENodeBFunction=1,EUtraNetwork=1,ExternalENodeBFunction=$configLTEnodename,$picotype=$configLTEnodecellname\",attributes=\"$masterEUtranCellFDDId (string)=$configLTEnodecellname\";",
"setmoattribute:mo=\"ManagedElement=$LTENAME,ENodeBFunction=1,EUtraNetwork=1,ExternalENodeBFunction=$configLTEnodename,$picotype=$configLTEnodecellname\",attributes=\"$localCellId (int32)=$currlocalCellId\";"
    );# end @MMLCmds

    $NETSIMMMLSCRIPT=&makeMMLscript("append",$MMLSCRIPT,@MMLCmds);
    #print "DEBUG $configLTEnodecellname $localCellId\n";
    $picoarrayposition++;
