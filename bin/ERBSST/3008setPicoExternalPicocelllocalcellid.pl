...
# Purpose     : sets PICO2PICO External.localCellId              
...
while ($NODECOUNT<=$PICONUMOFRBS){
    ...
    while($counter<=$PICORELATIONSNUMBER){
  	  ...
  	  "setmoattribute:mo=\"ManagedElement=$LTENAME,ENodeBFunction=1,EUtraNetwork=1,ExternalENodeBFunction=$tempstringnode,$picotype=$tempstringnode-$cellcount\",attributes=\"$localCellId (int32)=1\";" 
    );# end @MMLCmds
