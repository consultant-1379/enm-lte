...
# Purpose     : creates PICO ExternalENodeBFunction for LTE             
...
# Purpose     : SNAD is trying to set masterEnbFunctionId on proxies 
#               as they are different from its master's ENodeBFunctionId
# Description : Setting MasterEnBFunctionId to 1 for all LTE Proxies as
#               SNAD is setting them to 1
# Jira 	      : NETSUP-3523
...
local $CELLNUM;
local $minimumcellsize=3; # minimum LTE node cell size to be related with PICO nodes
...
while ($NODECOUNT<=$PICONUMOFRBS){
   ...
   while ($LTENAME=~m/$configPICOnodename/){ # match PICO node 
    ...
    # check if external enodebfunction LTE used already
    if($matchcounter>1){
    ...
    if($prevLTENodename=~m/$configLTEnodename/){
       $picoarrayposition++;next;}
...
"createmo:parentid=\"ManagedElement=$LTENAME,ENodeBFunction=1,EUtraNetwork=1\",type=\"$picotype\",name=\"$configLTEnodename\";",
"setmoattribute:mo=\"ManagedElement=$LTENAME,ENodeBFunction=1,EUtraNetwork=1,$picotype=$configLTEnodename\",attributes=\"masterEnbFunctionId  (String)=1\";"
    );# end @MMLCmds
