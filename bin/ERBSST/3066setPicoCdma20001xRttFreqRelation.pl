...
# Jira        : NSS-1954
# Purpose     : To redcue number of cdma2000 and cdma200001 relations
#               and proxies
...
$currCdma20001xRttFreqBandRelation=1;
    while ($currCdma20001xRttFreqBandRelation<=$totalCdma20001xRttFreqBandRelation)
    {
        ...
        if ($newcounter < 3) {$currCdma2000FreqBand=1;}
        if ($newcounter > 2 && $newcounter < 5) {$currCdma2000FreqBand=2;}
        if ($newcounter > 4 && $newcounter < 7) {$currCdma2000FreqBand=3;}
        if ($newcounter > 6 && $newcounter < 9) {$currCdma2000FreqBand=4;}
        if ($newcounter > 8 && $newcounter < 11) {$currCdma2000FreqBand=5;}
        if ($newcounter > 10 && $newcounter < 13) {$currCdma2000FreqBand=6;}


                        # build mml script
			$mmlstring="setmoattribute:mo=\"ManagedElement=$LTENAME,ENodeBFunction=1,EUtranCellFDD=$LTENAME-$cellcount,$Cdma20001xRttBandRelation=$currCdma20001xRttFreqBandRelation,$Cdma20001xRttFreqRelation=$currCdma20001xRttFreqRelation\",
attributes=\"$cdma2000FreqRef (moRef)=ManagedElement=$LTENAME,$extCdmaFreqPath,$Cdma2000FreqBand=$currCdma2000FreqBand,$Cdma2000Freq=$currCdma20001xRttFreqRelation\";";
