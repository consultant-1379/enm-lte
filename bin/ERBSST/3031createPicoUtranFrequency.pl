...
# Purpose     : creates PICO UtraNetwork.UtranFrequency             
...
while ($NODECOUNT<=$PICONUMOFRBS){
    ...
    while($currUtranFrequency<=$TotalUtranFrequency){# start while
       ...
       @MMLCmds=(".open ".$SIMNAME,
          ...
          "createmo:parentid=\"ManagedElement=$LTENAME,ENodeBFunction=1,UtraNetwork=1\",type=\"$picotype\",name=\"$currUtranFrequency\";" 
       );# end @MMLCmds
