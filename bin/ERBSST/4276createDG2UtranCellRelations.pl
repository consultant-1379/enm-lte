...
# Jira        : OSS-77902
# Purpose     : Creates UtranCellRelation and ExternalUtranCells
...
local $TOTALUTRANPEREUTRANCELL=5; #######3=6 NRM6.2
...
#TOTALUTRANPEREUTRANCELL
while($tempcellnum<=$TOTALUTRANPEREUTRANCELL){
   #------------------------------------------------
...
#####nrm6_2
   ...
   @MOCmds=qq^ CREATE
      (
    parent "ManagedElement=$LTENAME,ENodeBFunction=1,$TYPE=$ltecellname,UtranFreqRelation=$utranfreqrel"
    identity "$cid"
...
while($temputranfrequency<=$utranfrequency){ # while utranfrequency
 ...
 while($tempcellnum<=$STATICCELLNUM){ # while CELLNUM
   ...
   #------------------------------
    ...
    @MOCmds=qq^ CREATE
      (
      parent "ManagedElement=$LTENAME,ENodeBFunction=1,$TYPE=$LTENAME-$UtranCellRelationNum,UtranFreqRelation=$temputranfrequency"
