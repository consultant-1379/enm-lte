...
# Purpose     : Invalid plmnIds identified on SNAD
# Description : setting the plmnIds for PICO utran cells as SNAD is
#               throwing errors as they are not set
# Jira        : NETSUP-3523
...
while ($NODECOUNT<=$PICONUMOFRBS){
	...
	"setmoattribute:mo=\"ManagedElement=$LTENAME,ENodeBFunction=1,UtraNetwork=1,UtranFrequency=$currUtranFrequency,$picocelltype=$ExternalUtranCellFDDID\",attributes=\"plmnIdentity (struct)=[353,57,2]\";"
         );# end @MMLCmds
