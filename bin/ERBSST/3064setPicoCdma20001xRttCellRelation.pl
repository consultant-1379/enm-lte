...
local $MAXExternalCdma2000Cell=10909;
...
local $tempcounter;
local $totalexternalcellsperfreqband=5;
...
local $totalRttCellsPerNode=($totalCdma20001xRttFreqBandRelation*$totalCdma20001xRttFreqRelation*$totalCdma20001xRttCellRelation);
...
# get starting sim ExternalCdma2000CellRttID
$ExternalCdma2000CellID=($nodecountinteger*$totalRttCellsPerNode)-$Minus1totalRttCellsPerNode;

# ensure MaxExternalUtranCellFDD not exceeded
if($ExternalCdma2000CellID>$MAXExternalCdma20001xRttCell){$ExternalCdma2000CellID=$ExternalCdma2000CellID%$MAXExternalCdma2000Cell;}
...
while ($NODECOUNT<=$PICONUMOFRBS){
    ...
    #####################################
    # start get PICO node ExternalCdma2000Cell
    #####################################
    $arrcount=0;
    ...
    while ($currCdma2000FreqBand<=$totalCdma2000FreqBand){# while Cdma2000FreqBand
		...
		while($currCdma2000Freq<=$totalCdma2000Freq){# while Cdma2000Freq
			...
			while ($tempcounter<=$totalexternalcellsperfreqband){
				...
				$ExternalCdma20001xRttCellIDperNode[$arrcount]="ManagedElement=$LTENAME,ENodeBFunction=1,$Cdmanetwork=1,$Cdma2000FreqBand=$currCdma2000FreqBand,$Cdma2000Freq=$currCdma2000Freq,$ExternalCdma20001xRttCell=$ExternalCdma2000CellID";
...
$currCdma20001xRttFreqBandRelation=1;
    while ($currCdma20001xRttFreqBandRelation<=$totalCdma20001xRttFreqBandRelation)
    {
		...
		$currCdma20001xRttCellRelation=1;
                while ($currCdma20001xRttCellRelation<=$totalCdma20001xRttCellRelation)
                {

                        # build mml script
			$mmlstring="setmoattribute:mo=\"ManagedElement=$LTENAME,ENodeBFunction=1,EUtranCellFDD=$LTENAME-$cellcount,$Cdma20001xRttBandRelation=$currCdma20001xRttFreqBandRelation,$Cdma20001xRttFreqRelation=$currCdma20001xRttFreqRelation,$Cdma20001xRttCellRelation=$currCdma20001xRttCellRelation\",
attributes=\"$externalCdma20001xRttCellRef (moRef)=$ExternalCdma20001xRttCellIDperNode[$arrcount]\";";
