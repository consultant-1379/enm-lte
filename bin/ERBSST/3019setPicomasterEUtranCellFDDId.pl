...
# Purpose     : sets PICO masterEUtranCellFDDId in ExternalEUtranCellFDD            
# Description : creates support for setting masterEUtranCellFDDId 
#	        in PICO nodes. 
# Jira        : NETSUP-1019
# Date        : Jan 2014
# Who         : ecasjim
...
local $outercounter;
local $counter;
local $eutranFrequencyRef="eutranFrequencyRef";
...
while ($NODECOUNT<=$PICONUMOFRBS){
    ...
    while($counter<=$PICORELATIONSNUMBER){
    	  ...
    	  "setmoattribute:mo=\"ManagedElement=$LTENAME,ENodeBFunction=1,EUtraNetwork=1,ExternalENodeBFunction=$tempstringnode,$picotype=$tempstringnode-$cellcount\",attributes=\"$masterEUtranCellFDDId (string)=$tempstringnode-$cellcount\";"
    );# end @MMLCmds
